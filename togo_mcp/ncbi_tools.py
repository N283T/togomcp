...
def _validate_query_field_tags(query: str, database: str) -> dict[str, Any]:
    ...
    for term in organism_terms:
        if (
            re.search(rf"\b{term}\b", query, re.IGNORECASE)
            and "[Organism]" not in query
            and "[organism]" not in query
        ):
    ...
    if database == "gene":
        ...
        if potential_genes and "[Gene Name]" not in query:
            issues.append(
                "Potential gene symbols found without [Gene Name] tag: "
                f"{', '.join(set(potential_genes))}"
            )
...
def _format_esearch_result(
    data: dict[str, Any], db: str, query: str, validation: dict | None = None
) -> str:
    ...
    if normalized_db not in NCBI_DATABASES:
        ...
        return [
            TextContent(
                type="text",
                text=(
                    f"Error: Unsupported database '{database}'. "
...
async def ncbi_list_databases() -> list[TextContent]:
    ...
    result += (
        '  Example: ncbi_esearch(database="gene",'

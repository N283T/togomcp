...
from .server import (
    DBNAME_DESCRIPTION,
    ENDPOINT_NAME_TO_DATABASES,
    ENDPOINT_NAME_TO_URL,
    ENDPOINT_NAMES,
    MIE_DIR,
    SPARQL_ENDPOINT,
    TOGOMCP_USAGE_GUIDE,
...
def togomcp_usage_guide() -> str:
    """
    CALL THIS TOOL FIRST before using ANY other TogoMCP tool
    (SPARQL, search, or database tools).
    ...
    **CRITICAL**: 95% of query failures happen because users skip
      step 1 and use text search (bif:contains) or API calls when
      structured predicates exist in the schema. Skipping this
      wastes 10-20 tool calls and produces incomplete results.
      For comprehensive queries (counts, 'find all', 'which has
      most'), this guide shows you how to discover structured
      properties (taxonomy IRIs, typed predicates, classification
      terms) that are 10-100x faster than text search. Always call
...
@mcp.tool(
    ...
    description=(
        ...
        "Specify dbname for single-database queries, "
        "or endpoint_name/endpoint_url for cross-database "
        "queries on shared endpoints."
    ),
)
async def run_sparql(
    ...
    endpoint_name: Annotated[
        ...
        Field(
            description=(
                "Endpoint name for cross-database queries."
                f" One of: {', '.join(ENDPOINT_NAMES)}."
                " Use this when querying multiple databases"
    ...
    endpoint_url: Annotated[
        ...
        Field(
            description=(
                "Direct SPARQL endpoint URL. Use this for explicit control over the endpoint."
            ),
            default="",
        ),
    ] = "",
    ...
    Args:
        ...
        endpoint_name (str, optional): Endpoint name for
            cross-database queries (e.g., 'ebi' for ChEMBL+ChEBI).
...
@mcp.tool(
    name="get_MIE_file",
    description=(
        "**At the start of any task, identify ALL databases needed "
        "and call this tool for EACH of them before writing any "
        "SPARQL queries.** Do not query a database until its MIE "
        "file has been read. Get the MIE (Metadata Interoperability "
        "Exchange) file containing the ShEx schema, RDF and SPARQL "
        "examples of a specific RDF database."
    ),
)
...
def list_databases() -> list[dict[str, Any]]:
    ...
    **What it returns:**
    ...
    - Description (detailed content, entities, cross-references,
      use cases)
    ...
    **Common mistake to avoid:**
    ...
    Discovering databases by reading descriptions
    (e.g., "gene query -> both NCBI Gene AND Ensembl
    have complementary data")
